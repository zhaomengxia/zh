    ...
    public boolean hasPermission(HttpServletRequest request, Authentication authentication) {
        ...
        if (principal instanceof UserDetails) {
            try {
                ...
                //系统管理员拥有所有权限
                if (sysUser.getAuthorities().parallelStream().anyMatch(e -> "系统管理员".equals(e.getAuthority()))) {
                    return true;
                }
                ...
                for (SysRoles role : roles) {
                    urls.addAll(role.getResources().parallelStream().map(SysResources::getHttpPath).collect(Collectors.toSet()));
                }
